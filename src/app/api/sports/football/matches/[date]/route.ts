...
export const getMatches = async (
    ...
    // 1) Flatten but keep references to country + league
    type FlatEntry = {
        countryCode: string;
        countryName: string;
    ...
    flat.sort((a, b) => {
        if (a.leaguePriority && !b.leaguePriority) return -1;
        if (!a.leaguePriority && b.leaguePriority) return 1;
    ...
    const grouped: {
        [countryCode: string]: {
            ...
            countryCode: string;
            countryLogo?: string | null;
            leagues: { [leagueId: string]: { leagueId: any; leagueName: string; leagueLogo?: string | null; matches: any[] } };
        };
    } = {};
    ...
    const result: { [countryCode: string]: any } = {};

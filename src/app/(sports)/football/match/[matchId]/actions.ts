...
import { FootballMatchModel, IFMatch } from "@/models/Football/Match";
...
function isInProgress(state: MatchState): boolean {
    const inProgressStates: MatchState[] = [
        "First half",
        "Second half",
        "Extra time",
        "Break time",
...
const getMatch = async (matchId: number): Promise<IFootballMatch | null> => {
    const resp = await getMatchById_(matchId);

    if (!resp || resp.statusCode === 400 || resp.statusCode === 500) return null;

...
function safeJsonStringify(obj: any, maxDepth = 6, maxKeys = 300): string | null {
    try {
        ...
        try {
            const shallow = (function copyWithDepth(value: any, depth: number, maxKeys = 300): any {
                ...
                for (const k of keys) {
                    try {
                        out[k] = copyWithDepth((value as any)[k], depth - 1, maxKeys);
                    } catch {
                        out[k] = "[Error]";
                    }
                }

...
const ProfilePage = ({ username }: { username: string }) => {
    ...
    // Goat Deck Data
    const goatDackRef = useRef<HTMLDivElement>(null);
    ...
    const FavoriteCard = ({ src, label, hasCrown = false }: { src: string; label: string; hasCrown?: boolean }) => {
            ...
            <main className="flex flex-col pb-[10%] md:pb-[3%] w-full lg:max-w-[33.33vw] scrollbar-none md:scrollbar-thin- scrollbar-track-white scrollbar-thumb-gray-100 scrollbar-corner-gray-200 overflow-y-auto overflow-hidden h-screen">
                <section className="w-full">
                    ...
                    <div className="w-full max-h-[20vh] relative overflow-hidden rounded-b-md">
                        <img
                            ...
                            src={coverPhoto && coverPhoto.length > 0 ? coverPhoto : "/images/default_cover.svg"}
                        />

                        {(!coverPhoto || coverPhoto.length === 0) && (
                            <div className="absolute w-full h-full bg-black/30 top-0 left-0 flex items-center justify-center">
                                {!isMyProfile && <span className="text-sm text-white/70 font-medium">No cover photo</span>}

                                {isMyProfile && (
                                    <button
                                        onClick={(e) => router.push("/edit")}
                                        className="text-[#2C2C2C] bg-[#B9C9C6] font-semibold text-lg flex items-center w-12 aspect-square justify-center rounded-full  transition-all duration-200 shadow-lg"
                                    >
                                        <TbCameraUp />
                                    </button>
                                )}
                ...
                <div className="flex md:ml-2 flex-col items-center- -translate-y-7 px-2 md:px-0 pl-3- md:pl-6- gap-4">
                    ...
                    <div className="flex items-center gap-4- w-full overflow-auto scrollbar-none justify-between-">
                        <Badge
                            logoSrc="https://upload.wikimedia.org/wikipedia/commons/thumb/6/67/Borussia_Dortmund_logo.svg/800px-Borussia_Dortmund_logo.svg.png"
                            title="Dortmund Fan"
                            subtitle="since 2004"
                        />
...
const TabsView = ({ isMyProfile, currentTab, setTab }: { currentTab: string; isMyProfile: boolean; setTab: (tab: string) => void }) => {
    const TabItem = ({ name, icon, onClick, isActive }: { name: string; icon?: React.ReactNode; onClick: (e?: React.MouseEvent<HTMLButtonElement>) => void; isActive: boolean }) => {

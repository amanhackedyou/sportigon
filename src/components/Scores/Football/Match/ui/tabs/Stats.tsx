...
const StatsTab = ({ isActive }: { isActive: boolean }) => {
    ...
    return (
        <div className="w-full pb-2 h-full flex flex-col gap-3  overflow-y-auto py-2- mt-1- px-1- scrollbar-thin scrollbar-track-white scrollbar-thumb-gray-100 scrollbar-corner-gray-200">
            <div className="flex items-center gap-5- justify-between bg-[#e5e5e5] px-3 py-2 rounded-">
                <button
                    onClick={(e) => setCurrrentTab("MATCH")}
                    className={`text-sm font-medium  px-2 py-1 rounded-md ${currrentTab === "MATCH" ? "bg-white" : ""
                        }`}
                >
                    MATCH
                </button>
                <button
                    onClick={(e) => setCurrrentTab("1ST HALF")}
                    className={`text-sm font-medium  px-2 py-1 rounded-md ${currrentTab === "1ST HALF" ? "bg-white" : ""
                        }`}
                >
                    1ST HALF
                </button>
                <button
                    onClick={(e) => setCurrrentTab("2ST HALF")}
                    className={`text-sm font-medium  px-2 py-1 rounded-md ${currrentTab === "2ST HALF" ? "bg-white" : ""
                        }`}
                >
                    2ND HALF
                </button>
...

            {
                matchContext.match?.statistics && matchContext.match.statistics.length > 0 ? matchContext.match.statistics[0].statistics.map((homeTeam, index) => {
                    ...
                    return <Line
                        ...
                        leftStatePerc={homeTeamValue && awayTeamValue ? Math.round((homeTeamValue / (homeTeamValue + awayTeamValue)) * 100) : 0}
                        rightStatePerc={homeTeamValue && awayTeamValue ? Math.round((awayTeamValue / (homeTeamValue + awayTeamValue)) * 100) : 0}
                        leftText={homeTeam.value.toString() || "0"}
                        rightText={awayTeam?.value.toString() || "0"} />
...
const Line = ({
    ...
    rightText,
    leftText,
}: { title: string, rightStatePerc: number | string, leftStatePerc: number | string, rightText: string, leftText: string }) => {
    ...
    return (
        <div className="flex flex-col px-2 gap-[0.15rem]">
            ...
            <div className="flex items-center justify-between gap-[0.10rem]">
                <div className="w-full h-2 bg-[#e5e5e5] rounded flex justify-end">
                    <div
                        style={{ width: leftStatePerc + "%" }}
                        className={`h-full ${leftStatePerc > rightStatePerc ? RED_COLOR : GREEN_COLOR
                            } rounded-sm`}
                    ></div>
                </div>
                <div className="w-full h-2 bg-[#e5e5e5] rounded-sm">
                    <div
                        style={{ width: rightStatePerc + "%" }}
                        className={`h-full ${leftStatePerc < rightStatePerc ? RED_COLOR : GREEN_COLOR
                            } rounded-sm`}
                    ></div>
                </div>

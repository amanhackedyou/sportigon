...
const EventRow = ({ event, homeTeamId, awayTeamId }: { event: EventItem, homeTeamId: number, awayTeamId: number }) => {
    return (
        <div>Goal</div>
    )
}
...
const Goal = ({ team, isSelfGloal = false, time = "", playerName, teamMateName = "" }: { team: string, isSelfGloal?: boolean, firstGoal: number, time: string, playerName: string, teamMateName: string }) => {
    if (isSelfGloal) {
        team = team === "a" ? "b" : "a";
    }

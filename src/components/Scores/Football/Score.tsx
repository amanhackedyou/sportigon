...
function getStageKey(stage: MatchStage, minute?: number, penaltyScore?: string): string {
    switch (stage) {
        case "Not started":
        ...
        case "Extra time":
            if (minute) {
                if (minute <= 105) return `ET ${minute}'`; // first 15
                if (minute > 105 && minute < 120) return `ET ${minute}'`; // second 15
                if (minute === 105) return "ET HT"; // halftime break in ET
            }
            return "ET";
...
export const LeagueSeprater = ({ countryIcon, leagueName, countryName }: { countryIcon: string; leagueName: string; countryName: string; }) => {
    return <div className="flex px-2 mt-8- mt-2">
        <div className="flex items-center gap-2 leading-none">
            <img
                src={countryIcon}
                className="h-5 rounded- border-[1px]-"
            />
    ...
    return (
        ...
        >
            ...
            <Link
                ...
                className="flex justify-between relative  bg-greyPrimary- border-[#fb4f4f]- border-[#666666] border- bg-greyThird- bg-[#676087] pl-2 py-2- rounded-xl cursor-pointer overflow-hidden- transition-all hover:bg-gray-200-"
            >
                ...
                <div className="flex justify-between rounded-r-xl text-[#444] rounded-l-sm pr-2 pl-1 py-2 bg-[#ececec] w-full">
                    <div className="flex  gap-4 items-center ">
                        ...
                        <span className="font text-xs text-black- font-medium">{getStageKey(stage as MatchStage, minute, penaltyScore)}</span>
                    </div>

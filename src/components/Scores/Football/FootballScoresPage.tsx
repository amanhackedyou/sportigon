...
const fetchMatches = async (date: string, timezone: string, page: number, limit = 50) => {
    return HttpManager.get(`/api/sports/football/matches/${date}?timezone=${timezone}&page=${page}&limit=${limit}`);
}

function parseScore(score: string | null): { homeTeamScore: string; awayTeamScore: string } {
    if (!score) {
...
//     const [currentSelectedDate, setCurrentSelectedDate] = useState<string>(new Intl.DateTimeFormat("en-CA").format(new Date()));
//     const [currentPage, setCurrentPage] = useState(1);
//     const [hasMore, setHasMore] = useState(true);
//     const router = useRouter();

//     const { showModal } = useModal();
//     const loadMatches = async (currentPage_?: number) => {
//         const page = currentPage_ ?? currentPage;
...
//         setMatches(prev => page === 1 ? response.matches : { ...prev, ...response.matches });
//         setIsLoading(false);
...
//                     }
//                 }
//             }} className='w-full flex-1 h-full overflow-auto' >
//                 {
//                     matches && Object.keys(matches).length > 0 && (
//                         Object.keys(matches).map((countryCode) => {
//                             const country = matches[countryCode];
//                             return (
//                                 <div key={countryCode} className="mb-6">
//                                     {/* Loop through leagues inside the country */}
//                                     {country.leagues.map((league: any) => (
//                                         <div key={league.leagueId} className="mb-4">
//                                             <LeagueSeprater
//                                                 // countryIcon={country.countryLogo}
//                                                 countryIcon={league.leagueLogo || country.countryLogo}
//                                                 leagueName={league.leagueName}
//                                                 countryName={country.countryName}
//                                             />

//                                             {/* Matches inside the league */}
//                                             {league.matches.map((match: any, index: number) => {
//                                                 const score = parseScore(match.score?.current);
//                                                 return (
//                                                     <div key={index}>
//                                                         <Score
//                                                             stage={match.state || match.round}
//                                                             minute={match.clock}
//                                                             penaltyScore={match.score?.penalties}
//                                                             homeTeam={{
...
//                 {hasMore && <LoadingIndicator className='pb-6' />}
...
type Match = any; // refine if you want
type League = {
...
type MatchesMap = { [countryCode: string]: CountryCached };

const FootballScoresPage: React.FC = () => {
    const [matches, setMatches] = useState<MatchesMap>({});
    const [initialLoading, setInitialLoading] = useState<boolean>(true); // first page
    const [pageLoading, setPageLoading] = useState<boolean>(false); // subsequent pages
    ...
    const mergeMatches = useCallback((prev: MatchesMap, incoming: MatchesMap) => {
        // deep-merge countries -> leagues -> matches; avoid duplicate match ids
        const out: MatchesMap = { ...prev };
        ...
        for (const countryCode of Object.keys(incoming)) {
            ...
            const leaguesMap: { [id: string]: League } = {};
            ...
            for (const incomingLeague of incomingCountry.leagues) {
                ...
                if (!leaguesMap[lid]) {
                    ...
                    const existingIds = new Set(existingMatches.map((m: any) => m.id));
                    ...
                    existingMatches.sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());
                }
            }
    ...
    const loadMatches = useCallback(
        async (pageOverride?: number) => {
            ...
            try {
                ...
                const incoming = response.matches as MatchesMap;
            ...
            return (
                <div key={countryCode} className="mb-6">
                    {country.leagues.map((league) => (
                        <div key={String(league.leagueId)} className="mb-4">
                            <LeagueSeprater
                                countryIcon={league.leagueLogo || country.countryLogo || ""}
                                leagueName={league.leagueName}
                                countryName={country.countryName}
    ...
    return (
        <div className="hFull overflow-hidden flex flex-col">
            <SeconderyNavBar title="Scores" />
            <ScheduleSection currentDate={currentSelectedDate} />

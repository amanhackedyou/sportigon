...
const LoginPage = () => {
    ...
    return (<form onSubmit={e => e.preventDefault()} className={`flex-col h-screen w-full px-5 justify-center gap-2 flex`}>
            ...
            <input type="email" className="bg-gray-100 focus:bg-white rounded-md border border-gray-300 focus:border-[#007600] focus:ring-2 focus:ring-blue-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" placeholder="Email" value={Email} onChange={(e) => setEmail(e.target.value)}/>

            <hr />
            <input type="text" min={userConfig.minPasswordLength} max={userConfig.maxPasswordLength} required pattern={`^[a-zA-Z0-9_]{${userConfig.minPasswordLength},${userConfig.maxPasswordLength}}$`} title={`Password must be between ${userConfig.minPasswordLength} and ${userConfig.maxPasswordLength} characters long and can only contain letters, numbers, and underscores.`} className=" bg-gray-100 focus:bg-white rounded-md border border-gray-300 focus:border-[#007600] focus:ring-2 focus:ring-blue-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" placeholder="Password" value={Password} onChange={(e) => setPassword(e.target.value)}/>
            ...
            <BigButton onClick={handleLogin} text="Login" isActive={validateForm()} isProcessing={isLogging}/>

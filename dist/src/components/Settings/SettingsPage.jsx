...
const SettingsPage = () => {
    ...
    const [coverPhotoEditFile, setCoverPhotoEditFile] = useState(null);
    const DESCRIPTION_MAX_LENGTH = 90;
    // Favorite Player Data
    const [favoritePlayer, setFavoritePlayer] = useState([]);
    const [isSelectFavoritePlayerWindowOpened, setIsSelectFavoritePlayerWindowOpened,] = useState(false);
    // Favorite CLub Data
    const [favoriteClubs, setFavoriteClubs] = useState([]);
    const [isSelectFavoriteClubsWindowOpened, setIsSelectFavoriteClubsWindowOpened,] = useState(false);
    // Countries Data
    const [selectedCountries, setSelectedCountries] = useState([]);
    const [isSelectCountriesWindowOpened, setIsSelectCountriesWindowOpened] = useState(false);
    // Edit account
    const [username, setUsername] = useState("");
    const [email, setEmail] = useState("");
    // ~ Password Window Login
    const [isChangePasswordWindowOpened, setIsChangePasswordWindowOpened] = useState(false);
    ...
    const coverPhotoUploaderRef = useRef(null);
    return (<div className='w-full h-full'>
            {/* <SeconderyNavBar title="Edit your account and profile" /> */}
            {coverPhotoEditFile && (<CoverPhotoCropingWindow file={coverPhotoEditFile} onClose={() => setCoverPhotoEditFile(null)} onCropped={url => setCoverPhoto(url)}/>)}

            {isSelectFavoritePlayerWindowOpened && (<SelectFavoritePlayerWindow onPlayerSelect={(player) => {
                setFavoritePlayer(player);
                setIsSelectFavoritePlayerWindowOpened(false);
            }} setIsSelectFavoritePlayerWindowOpened={setIsSelectFavoritePlayerWindowOpened} alreadySelectedPlayers={favoritePlayer}/>)}


            {isSelectFavoriteClubsWindowOpened && (<SelectFavoriteClubsWindow onClubsSelect={(club) => {
                setFavoriteClubs(club);
                setIsSelectFavoriteClubsWindowOpened(false);
            }} alreadySelectedClubs={favoriteClubs} setIsSelectFavoriteClubsWindowOpened={setIsSelectFavoriteClubsWindowOpened}/>)}

            {isSelectCountriesWindowOpened && (<SelectFavoriteCountriesWindow onCountriesSelect={(countries) => {
                setSelectedCountries(countries);
                setIsSelectCountriesWindowOpened(false);
            }} countries={countries} alreadySelectedCountries={selectedCountries} setIsSelectFavoriteCountriesWindowOpened={setIsSelectCountriesWindowOpened}/>)}
            ...
            <section className='flex flex-col mt-px overflow-auto h-full'>
                        <div className="w-full max-h-[20vh]- h-[20vh]- aspect-video px-2 py-px relative overflow-hidden- rounded-b-md">
                            <div className='w-full relative h-full  overflow-hidden'>
                                <img className="w-full h-full object-cover rounded-lg -border-[0.7px] border-[#c1c1c1]- -shadow-[rgba(0,0,0,0.1)_0px_0px_8px_0px,rgba(0,0,0,0.1)_0px_0px_2px_0px]  opacity-[0.1]- p-px- border-[#e5e5e5]-  object-center" 
            ...
            src={coverPhoto}/>
                            ...
                            <div className='absolute -bottom-10 z-50- left-0 w-full px-4'>
                                <div className=' w-20 aspect-square relative rounded-full overflow-hidden- border-[4px] border-white shadow'>
                                    <img className="w-full h-full rounded-full  p-2- object-cover rounded-b-full-" src={profilePictureSrc} alt=""/>
                ...
                /> */}
                    ...
                    <ExpandableTextArea maxLength={DESCRIPTION_MAX_LENGTH} value={description}
                        ...
                        onChange={(e) => setDescription(e.target.value)} />
                                ...
                                <textarea id="profile_desc" name="profile_desc" value={description} onBeforeInput={e => {
                    if (description.length >= DESCRIPTION_MAX_LENGTH)
                        e.preventDefault();
                }} onChange={e => setDescription(e.target.value)} placeholder="Say something about yourself..." className="w-full bg-[#F2F2F2] bg-white- placeholder:text-[#808080] rounded-2xl border border-gray-300 focus:border-[#8ED500] focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-black py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out"></textarea>
            ...
            : <section className="flex flex-col mb-8 px-2  md:px-14 pt-10 h-full gap-1">
                ...
                value={username} placeholder={"Display name"} onChange={(e) => setUsername(e.target.value)}/>
                            <EditTextField name="Email" placeholder={"Email"} type='email' value={email} onChange={(e) => setEmail(e.target.value)}/>
                            <EditTextField name="Password" placeholder="Password" 
                // isPassword={true}
                // onClick={e => setIsChangePasswordWindowOpened(true)}
                onClick={openPasswordWindow} value="*******" isDisabled={true}/>
                            <button className="text-white bg-[#4DD21D]- bg-[#007600] hover:bg-[#007600b1] border-0 py-1 font-medium px-8 focus:outline-none  rounded-md text-lg">
                                Save
                            </button>
                        </div> :
                    ...
                    value={oldPassword} type="password" placeholder={"Current password"} createBottomSpace={false} onChange={(e) => setOldPassword(e.target.value)}/>
                                ...
                                <EditTextField name="Enter new password" value={newPassword} type="password" placeholder={"New password"} onChange={(e) => setNewPassword(e.target.value)}/>

                                <EditTextField name="Confirm new password" value={confirmNewPassword} type="password" placeholder={"Confirm password"} onChange={(e) => setConfirmNewPassword(e.target.value)}/>
                        ...
                        >
                            ...
                            <span>Back</span>
                                    ...
                                    <button onClick={closePasswordWindow} className={`text-white bg-[#4DD21D]- bg-[#007600] w-full hover:bg-[#007600b1] border-0 py-1 flex items-center justify-center gap-1  font-medium focus:outline-none  rounded-md text-lg`}>
                                        <IoMdArrowRoundBack className="text-xl"/>
                                        <span>Back</span>
                                    </button>
                                    ...
                                    <button className={` ${isValidPasswords() ? 'opacity-100 hover:bg-[#007600b1]' : 'opacity-50'} text-white bg-[#4DD21D]- bg-[#007600] w-full hover:bg-[#007600b1] border-0 py-1 font-medium focus:outline-none  rounded-md text-lg`}>
                                        Save change
                                    </button>
                                    {/* <button
                            onClick={(e) => {
...
const EditTextField = ({ name, value, onChange, placeholder, type = "text", isDisabled = false, createBottomSpace = true, onClick = () => { } }) => {
    const inputFieldRef = useRef(null);
    return (<div className={`relative  ${createBottomSpace && 'mb-4'} w-full`}>
            <p className="leading-7- leading-[1px]- leading-none mb-1 text-sm  text-black">{name}</p>
            <div className="flex items-center justify-between">
                <input type={type} onClick={e => {
            e.preventDefault();
            onClick();
        }} onChange={onChange} ref={inputFieldRef} 
    // onBlur={onBlur}
    // disabled={!isFocused}
...
const TabView = ({ currentTab, setTab }) => {
...
const FavoriteCard = ({ isCountry = false, src, label, hasCrown = false }) => {
...
const SelectFavoritePlayerWindow = ({ setIsSelectFavoritePlayerWindowOpened, onPlayerSelect, alreadySelectedPlayers }) => {
    const [players, setPlayers] = useState([
        {
            name: "Messi",
            photo: "https://media.cnn.com/api/v1/images/stellar/prod/230816151359-lionel-messi-inter-miami.jpg?c=16x9&q=h_720,w_1280,c_fill",
        },
        {
            name: "Ronaldo",
            photo: "https://media.vov.vn/sites/default/files/styles/large/public/2023-08/roo.jpg",
        },
        {
            name: "Mbappe",
            photo: "https://icdn.football-espana.net/wp-content/uploads/2023/06/Kylian-Mbappe-PSG-042922-169.jpg",
        },
        {
            name: "Neymar",
        ...
        {
            name: "Haaland",
            photo: "https://i.guim.co.uk/img/media/738ce55b0210242ed89525dc7217389fcdd954f6/867_295_3097_1858/master/3097.jpg?width=1200&height=1200&quality=85&auto=format&fit=crop&s=93c023f0b3182e74b5320381712d32d3",
        },
        {
            name: "Debruyne",
            photo: "https://images2.minutemediacdn.com/image/upload/c_crop,w_3946,h_2219,x_0,y_86/c_fill,w_720,ar_16:9,f_auto,q_auto,g_auto/images/GettyImages/mmsport/90min_en_international_web/01gtvkd1x49s2gddqzj2.jpg",
        },
        {
            name: "Kroos",
            photo: "https://i.eurosport.com/2020/05/15/2818998-58144988-2560-1440.jpg",
        },
        {
            name: "Rashford",
            photo: "https://staticg.sportskeeda.com/editor/2023/07/2a98b-16893358639379-1920.jpg?w=840",
        },
        {
            name: "Saka",
            photo: "https://talksport.com/wp-content/uploads/sites/5/2023/03/arsenals-english-midfielder-bukayo-saka-804253779.jpg",
        },
        {
            name: "R. James",
            photo: "https://e0.365dm.com/23/08/2048x1152/skysports-reece-james-chelsea_6250027.jpg",
        },
        {
            name: "Lewandowski",
            photo: "https://www.fcbarcelona.com/photo-resources/2022/11/02/85247947-27dc-488c-a6cb-d81c5e391559/09-ROBERT_LEWANDOWSKI.jpg?width=1200&height=750",
        },
        {
            name: "Gnabry",
            photo: "https://assets.bundesliga.com/tachyon/sites/2/2022/07/fcb_gnabry_1920.jpg?crop=0px,0px,1920px,1079px&fit=1140,1140",
        },
        {
            name: "Messi2",
            photo: "https://media.cnn.com/api/v1/images/stellar/prod/230816151359-lionel-messi-inter-miami.jpg?c=16x9&q=h_720,w_1280,c_fill",
    ...
    const [tempSelectedIndexs, setTempSelectedIndexs] = useState([]);
    ...
    console.log(tempSelectedIndexs);
    return (<OverlayWindowTemplate onClose={() => setIsSelectFavoritePlayerWindowOpened(false)} isYCenter={false} actions={<>
                    <button onClick={(e) => {
                ...
                : "opacity-60 cursor-not-allowed"} bg-[#007600] text-white font-medium px-3 py-2  rounded-full leading-none justify-center `}>
                        Save
                    </button>
            ...
            <div className=" md:px-5 w-full h-full overflow-hidden">
                <section className='px-2 flex flex-col pt-5 pb-3 gap-1 border-b'>
                    ...
                    <div className="flex items-center w-full py-2 px-3 mt-3 rounded-md gap-2 bg-[#f0f0f0]">
                        ...
                        <input placeholder="Search your favorite player..." className="w-full bg-transparent outline-none font-normal text-sm h-full text-[#373737] placeholder:text-[#777]" type="text"/>
                    </div>
                </section>
            ...
            return (<div onClick={(e) => {
                    ...
                    if (tempSelectedIndexs.length >= 11)
                        return;
                    ...
                    : ""} ${tempSelectedIndexs.length >= 11 && !tempSelectedIndexs.includes(i) ? 'opacity-50' : ''}`} key={i}>
                                    ...
                                    <img className="w-full h-[80%]- h-full object-cover object-center " src={player.photo} alt=""/>
...
const SelectFavoriteClubsWindow = ({ setIsSelectFavoriteClubsWindowOpened, onClubsSelect, alreadySelectedClubs }) => {
    const [clubs, setclubs] = useState([
        {
            name: "Man City",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/e/eb/Manchester_City_FC_badge.svg/1200px-Manchester_City_FC_badge.svg.png",
        },
        {
            name: "Chelsea",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/c/cc/Chelsea_FC.svg/800px-Chelsea_FC.svg.png",
        },
        {
            name: "Man United",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/7/7a/Manchester_United_FC_crest.svg/1200px-Manchester_United_FC_crest.svg.png",
        },
        {
            name: "Liverpool",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/0/0c/Liverpool_FC.svg/800px-Liverpool_FC.svg.png",
        },
        {
            name: "Tottenham",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/b/b4/Tottenham_Hotspur.svg/1200px-Tottenham_Hotspur.svg.png",
        },
        {
            name: "Barcelona",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/4/47/FC_Barcelona_%28crest%29.svg/640px-FC_Barcelona_%28crest%29.svg.png",
        },
        {
            name: "Real Madrid",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/5/56/Real_Madrid_CF.svg/1200px-Real_Madrid_CF.svg.png",
        },
        {
            name: "Sevilla",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/3/3b/Sevilla_FC_logo.svg/640px-Sevilla_FC_logo.svg.png",
        },
        {
            name: "Atletico Madrid",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/f/f9/Atletico_Madrid_Logo_2024.svg/1200px-Atletico_Madrid_Logo_2024.svg.png",
        },
        {
            name: "Bayern Munich",
            photo: "https://upload.wikimedia.org/wikipedia/commons/thumb/8/8d/FC_Bayern_M%C3%BCnchen_logo_%282024%29.svg/1200px-FC_Bayern_M%C3%BCnchen_logo_%282024%29.svg.png",
        },
        {
            name: "Borussia Dortmund",
            photo: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/67/Borussia_Dortmund_logo.svg/800px-Borussia_Dortmund_logo.svg.png",
        },
        {
            name: "Leipzig",
        ...
        {
            name: "Shalke 04",
            photo: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6d/FC_Schalke_04_Logo.svg/1200px-FC_Schalke_04_Logo.svg.png",
        },
        {
            name: "Stuttgart",
            photo: "https://upload.wikimedia.org/wikipedia/commons/thumb/e/eb/VfB_Stuttgart_1893_Logo.svg/1200px-VfB_Stuttgart_1893_Logo.svg.png",
        },
        {
            name: "Inter Millan",
            photo: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTFNcUnb49zp7qriWxPGn8xTAYQAFvIKLddRA&s",
        },
        {
            name: "Juventus",
            photo: "https://e7.pngegg.com/pngimages/177/345/png-clipart-juventus-logo-juventus-f-c-serie-a-juventus-stadium-football-uefa-champions-league-football-text-sport-thumbnail.png",
        },
        {
            name: "Napoli",
            photo: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTDYH_vR7azQimMDmhJOMk3dTt7mqz3GN3w1w&s",
        },
        {
            name: "Atalanta",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/6/66/AtalantaBC.svg/640px-AtalantaBC.svg.png",
        },
        {
            name: "Paris Saint Germain",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/a/a7/Paris_Saint-Germain_F.C..svg/1200px-Paris_Saint-Germain_F.C..svg.png",
        },
        {
            name: "Monaco",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/c/cf/LogoASMonacoFC2021.svg/1200px-LogoASMonacoFC2021.svg.png",
        },
        {
            name: "Marseille",
            photo: "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d8/Olympique_Marseille_logo.svg/1200px-Olympique_Marseille_logo.svg.png",
        },
        {
            name: "Reims",
            photo: "https://upload.wikimedia.org/wikipedia/en/thumb/1/19/Stade_de_Reims_logo.svg/1200px-Stade_de_Reims_logo.svg.png",
        },
    ]);
    const [tempSelectedIndexs, setTempSelectedIndexs] = useState([]);
    // console.log(tempSelectedIndexs);
    ...
    // console.log(tempSelectedIndexs);
    return (<OverlayWindowTemplate onClose={() => setIsSelectFavoriteClubsWindowOpened(false)} isYCenter={false} actions={<>
                    <button onClick={(e) => {
                if (tempSelectedIndexs.length > 0) {
                    const selectedClubs = [];
        ...
        <div className="flex items-center w-full py-2 px-3 rounded-full mt-2 gap-2 bg-[#E5E5E5]">
            ...
            return (<div onClick={(e) => {
                    ...
                    if (tempSelectedIndexs.length >= 3)
                        return;
                    setTempSelectedIndexs([...tempSelectedIndexs, i]);
                    ...
                    ? "border-[3.5px] border-[#4DD21D]"
                    : ""} ${tempSelectedIndexs.length >= 3 && !tempSelectedIndexs.includes(i) ? 'opacity-50' : ''}`} key={i}>
                                    {(tempSelectedIndexs.length > 0 && tempSelectedIndexs[0] === i) && <div className="bg-[#00000071] w-5 text-sm aspect-square leading-none absolute flex items-center justify-center rounded-md top-1 left-1"><span className="leading-none">👑</span></div>}
                                    <img className="w-full- h-full- h-[75%] object-cover object-center " src={club.photo} alt=""/>
...
const SelectFavoriteCountriesWindow = ({ setIsSelectFavoriteCountriesWindowOpened, onCountriesSelect, alreadySelectedCountries, countries }) => {
    ...
    // console.log(tempSelectedIndexs);
    return (<OverlayWindowTemplate onClose={() => setIsSelectFavoriteCountriesWindowOpened(false)} isYCenter={false} actions={<>
                    <button onClick={(e) => {
                if (tempSelectedIndexs.length > 0) {
            ...
            <div className="md:px-5 w-full h-fulll overflow-hidden">
                <section className='px-2 flex flex-col pt-5 pb-3 gap-1 border-b'>
                    ...
                    <div className="flex items-center w-full py-2 px-3 mt-3 rounded-md gap-2 bg-[#f0f0f0]">
                        <IoSearchOutline className="text-xl text-[#777]"/>
                        <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search your favorite player..." className="w-full bg-transparent outline-none font-normal text-sm h-full text-[#373737] placeholder:text-[#777]" type="text"/>
                    </div>
                </section>
            ...
            return (<div onClick={(e) => {
                    ...
                    : ""} ${tempSelectedIndexs.length >= 3 && !tempSelectedIndexs.includes(i) ? 'opacity-50' : ''}`} key={i}>
                                    ...
                                    <img className="rounded- w-full shadow-sm-  max-w-24 border-b object-cover object-center " src={country.flag_4x3} alt=""/>
...
const CoverPhotoCropingWindow = ({ file, onCropped = () => { }, onClose = () => { } }) => {
    ...
    const handleCrop = () => {
        ...
        ctx?.drawImage(image, 0, cropY, cropWidth * scale, cropHeight * scale, 0, 0, cropWidth * scale, cropHeight * scale);
        canvas.toBlob((blob) => {
            if (!blob)
                return;
    ...
    if (!imageUrl)
        return null;
    return (<section className="w-full h-full fixed top-0 left-0 bg-black bg-opacity-80 z-[53] flex items-center justify-center px-4">
            <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full relative p-4 flex flex-col items-center">
                <div className="w-full aspect-[16/9] overflow-y-scroll rounded-md border border-gray-300 relative" ref={imageContainerRef}>
                    {imageUrl && (<img src={imageUrl} ref={imageRef} alt="To crop" className="w-full select-none pointer-events-none" draggable={false}/>)}
                </div>

                <div className="flex gap-4 mt-6">
                    <button onClick={handleCrop} className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md font-semibold transition">
                        Crop
                    </button>
                    <button onClick={onClose} className="px-6 py-2 bg-gray-400 hover:bg-gray-500 text-white rounded-md font-semibold transition">
                        Cancel
                    </button>
                </div>
            </div>

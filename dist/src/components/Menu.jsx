...
import { TbLogout } from "react-icons/tb";
const Menu = ({ isMenuOpened, setIsMenuOpened, isLoggedIn, }) => {
    const pathname = usePathname();
    const router = useRouter();
    console.log(isLoggedIn);
    const LinkItemWithIcon = ({ href, icon, activeIcon, bedge = null, children, }) => {
        ...
        return (<button onClick={() => {
                ...
                <div className="text-2xl text-black">{icon}</div>
                <span className="text-black text-base font-rubik">{children}</span>
                {bedge && (<span className="ml-auto w-5 h-5 flex items-center justify-center text-xs text-white bg-[#007600] rounded-full">
                        {bedge}
                    </span>)}
            </button>);
    };
    ...
    return (<div className={`fixed  inset-0- z-[60] h-[calc(100%-3.5rem)] left-0 bottom-0 w-full transition-all duration-300 ${isMenuOpened ? "visible opacity-100" : "invisible opacity-0"}`}>
            {/* Backdrop */}
            <div className="absolute inset-0 bg-black/60" onClick={() => setIsMenuOpened(false)}/>

            {/* Sidebar */}
            <div ref={menuRef} className={`absolute left-0 top-0 bottom-0 w-[70%] bg-white flex flex-col justify-between transition-transform duration-300 ease-in-out ${isMenuOpened ? "translate-x-0" : "-translate-x-full"}`}>
                <div className="p-3 flex flex-col gap-[2px]">
                    ...
                    {isLoggedIn && (<LinkItemWithIcon href="/inbox" bedge={6} icon={<MessageIcon />} activeIcon={<RiMessage3Fill />}>
                            Messages
                        </LinkItemWithIcon>)}

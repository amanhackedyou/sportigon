...
const AddNewPost = ({ forceShow = false }) => {
    ...
    // ~ References
    const colorCardContainerRef = useRef(null);
    ...
    return (<>
            ...
            boxShadow: "rgba(0, 0, 0, 0.12) 0px 2px 8px 0px, rgba(0, 0, 0, 0.16) 0px 0px 2px 0px",
        }} className={`${forceShow ? "flex" : "hidden md:flex"}  flex-col w-full items-start gap-4 mt-3 py-2 rounded-xl  `}>
                <div className={`w-full flex gap-4 px-4- items-center px-4 ${isActive ? "justify-between" : ""}`}>
                    ...
                    {!isActive && (<div className="outline-none w-full font-semibold text-[#777777] text-[1.1rem]" onClick={(e) => setActivity(true)}>
                            What&#39;s on your mind?
                        </div>)}
                    {isActive ? (<button className="text-[#575757] hover:text-black" onClick={(e) => setActivity(false, true)}>
                            <IoClose className="text-3xl"/>
                        </button>) : ("")}
                </div>

                <section className={` w-full flex-col min-h-[2rem] relative ${!isValidBackgroundPost() ? "px-4" : "px-0"} ${isActive ? "flex" : "hidden"}`}>
                    ...
                    {!caption && (<p className={` absolute pointer-events-none w-full ${isValidBackgroundPost()
                ...
                : "text-[#777777] text-[1.1rem] font-semibold"}`}>
                    ...
                    <div onClick={(e) => inputFieldRef.current?.focus()} style={{
            ...
            : "h-full font-normal"}`}>
                        <div ref={inputFieldRef} onBlur={(e) => {
            if (!caption) {
                setActivity(false);
            }
        }} className={`outline-none w-full ${isValidBackgroundPost() ? "text-2xl" : ""}`} 
    //@ts-ignore
    onInput={(e) => onInput(e.target.innerText)} contentEditable></div>
                    </div>
                </section>

                <div className="flex w-full flex-col gap-1 pl-4">
                    {isActive && (<div className="flex gap-6 justify-start pml-[3.750rem] w-full text-[1.37rem] text-secondary">
                    ...
                    <div className=" w-full flex justify-between  items-center pr-2">
                        {isActive ? (<div onMouseEnter={(e) => setCanActiveChangable(false)} onMouseLeave={(e) => setCanActiveChangable(true)} className="flex items-center  gap-1 max-w-[50%]">
                                ...
                                <div ref={colorCardContainerRef} className="flex py-2  items-center--- scroll-smooth overflow-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-gray-300 justify-start w-full gap-2">
                                    <div style={{
                background: "white",
            }} onClick={(e) => {
                setCurrentBackgroundChoosen(null);
            }} className={`${currentBackgroundChoosen === null ? "border-2" : ""}  aspect-square border-[#585858] shadow-md min-w-[32px] cursor-pointer flex items-center justify-center rounded-lg`}></div>
                ...
                return (<div key={i} style={{
                        ...
                        setCurrentBackgroundChoosen(theme);
                    }} className={`${currentBackgroundChoosen === theme ? "border-2" : ""}  aspect-square border-[#585858] shadow-md min-w-[32px] cursor-pointer flex items-center justify-center rounded-lg`}></div>);
            })}
                                </div>
                            ...
                            </div>) : (<div className="flex gap-6  justify-start pml-[3.750rem] w-full text-[1.37rem] text-secondary">
                                <div className="cursor-pointer transition-all p-1 rounded-full hover:bg-[#ffe26f3a]">
                                    <HiOutlinePhotograph className="text-[#eb892e]"/>
                                </div>
                                <div className="cursor-pointer transition-all p-1 rounded-full hover:bg-[#6fc5ff3a]">
                        ...
                        <div className="flex gap-3 pr-2">
                            <div className="relative min-w-[7rem]" onMouseEnter={(e) => setIsCategoryDropdownOpened(true)} onMouseLeave={(e) => setIsCategoryDropdownOpened(false)}>
            ...
            return (<p key={category} onClick={(e) => {
            ...
            <Link href="/new" className={`${forceShow ? "hidden" : "flex md:hidden"} items-center w-full bg-red-100- border-[rgba(0,0,0,0.1)] border-b py-4 gap-2 mx-2- px-4 mt-3- justify-between- border- bg-greyThird-`}>

                <img className="w-9 h-9 object-cover object-center rounded-full" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSyW32Rrz3qcEezEab0N6p6fCHO1HeHDDNXXA&usqp=CAU"/>

                <div className="flex items-center mx-2- gap-2 pl-2 pr-3 w-full justify-between border- rounded-full bg-[#ececec]">
                    <div className="flex px-1   py-2 items-center gap-2 rounded-md  w-full">
...
//# sourceMappingURL=AddNewPost.jsx.map
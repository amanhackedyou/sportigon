...
//     url?: string;
// }
const MediaView = ({ mediaFiles, setMediaFiles }) => {
...
const ImageView = ({ file, onClear }) => {
    ...
    return (<>
            ...
            <section onClick={e => {
            //@ts-ignore
            if (e.target.tagName === "SECTION" || e.target.tagName === "IMG")
                ...
                <div className="border-2 border-white rounded-full w-5 overflow-hidden aspect-square text-white  absolute -right-2 -top-2">
                    <button onClick={onClear} className="bg-black bg-opacity-50 w-full h-full backdrop-blur-md  text-xl flex items-center justify-center">
...
const GifViewer = ({ url, file, onClear }) => {
    const [blobFileUrl, setBlobFileUrl] = useState(url);
    const [isFullScreen, setIsFullScreen] = useState(false);
    if (file) {
        const fileReader = new FileReader();
        fileReader.addEventListener("load", () => {
...
const FullScreenVideoView = ({ src, close }) => {
    ...
    const videoRef = useRef(null);
    const getVideoDuration = (position) => {
    ...
    return (<section onClick={(e) => {
            ...
            <MdOutlineClose onClick={(e) => close()} className="text-white absolute text-3xl top-4 right-3 md:top-16 md:left-16 cursor-pointer"/>

            <div className="max-w-[95%] md:max-w-[70%] relative rounded overflow-hidden ">
                <video muted={isMuted} autoPlay onLoadedData={(e) => {
            //@ts-ignore
            e.target.play();
        }} onTimeUpdate={(e) => {
            //@ts-ignore
            ...
            setIsPlaying(false);
        }} ref={videoRef} className="max-h-[80vh] md:max-h-[90vh] rounded-md" src={src}></video>

                {isBuffering && (<div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                ...
                <div className="absolute bg-gradient-to-t from-[#0000001b] to-[#0000004d] bottom-0 w-full py-2 overflow-hidden md:py-4 text-white px-2 md:px-4 flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        {!isPlaying ? (<FaPlay onClick={(e) => {
                videoRef.current?.play();
            }} className="text-lg md:text-xl cursor-pointer"/>) : (<FaPause onClick={(e) => {
        ...
        }} className="h-full bg-[#8ED500]"></div>
                        </div>

                        {isMuted ? (<HiSpeakerXMark onClick={(e) => setIsMuted(false)} className="text-xl md:text-2xl cursor-pointer"/>) : (<HiSpeakerWave onClick={(e) => setIsMuted(true)} className="text-xl md:text-2xl cursor-pointer"/>)}
                    </div>
                </div>
            </div>
        </section>);
};
const ShowMediaInFullScreen = ({ mediatype, src, onClose }) => {
    return <section onClick={e => {
            //@ts-ignore
            if (e.target.tagName === "SECTION")
                onClose();
...
//# sourceMappingURL=MediaView.jsx.map
...
const GifSelectorWindow = ({ close, onGifClick }) => {
    ...
    const selectGifFromGalaryRef = useRef(null);
    const gf = new GiphyFetch("fWm9VaWWqicDKoMzCKUmiTKINZAoAUIT");
    const getGifs = async (query, limit = 50) => {
        setIsLoading(true);
    ...
    const GifView = ({ src }) => {
        ...
        <img onClick={(e) => {
                onGifClick(src);
                close();
            }} alt="GIF image" onLoad={(e) => setIsLoaded(true)} className={`${!isLoaded ? "hidden" : "inline-block"} rounded-md cursor-pointer`} src={src}/>

        <div className={`${isLoaded ? "hidden" : "block"} bg-[#e5e5e5] aspect-square h-full`}></div>
      </div>);
    };
    return (<>
      ...
      <OverlayWindowTemplate isYCenter={false} onClose={close}>
        <div className="flex flex-col w-full h-full px-3 overflow-hidden gap-2">
          <div className="flex items-center gap-2 w-full py-2 px-3 rounded-full mt-2 bg-[#E5E5E5]">
            ...
            <input placeholder="Search all the GIFs" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-transparent outline-none text-sm h-full text-[#373737] placeholder:text-[#777]" type="text"/>
          </div>
          {isLoading && (<div className="w-full flex justify-center items-center mt-3">
              <div className="w-10 aspect-square border-b-2 border-gray-900 rounded-full animate-spin"></div>
            </div>)}

          <div className="py-2 max-h-full">
            <div className={` ${searchQuery !== "" ? "hidden" : "grid"} grid-cols-2 pb-5 md:grid-cols-3 gap-2 w-full  max-h-full overflow-auto scrollbar-thin scrollbar-track-white scrollbar-thumb-gray-100 scrollbar-corner-gray-200`}>
              <button onClick={(e) => {
            try {
            ...
            // close();
        }} className={`bg-[#e5e5e5] max-h-32 rounded-md gap-3 hover:bg-[#cccccc] min-h-32 ${!isLoading ? "flex" : "hidden"} flex-col items-center justify-center`}>
            ...
            <div className={` ${searchQuery !== "" ? "grid" : "hidden"} grid-cols-2 pb-5 md:grid-cols-3 gap-2 w-full  max-h-full overflow-auto scrollbar-thin scrollbar-track-white scrollbar-thumb-gray-100 scrollbar-corner-gray-200`}>
              {searchedGifs.map((gifUrl, i) => {
            return <GifView src={gifUrl} key={i}/>;
        })}
            </div>
            {isSearchNotFound && !isLoading && (<p className="text-center text-sm md:text-base">
...
//# sourceMappingURL=GifSelectorWindow.jsx.map
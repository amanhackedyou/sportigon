...
import { BsFillCalendarDayFill } from "react-icons/bs";
import { MdKeyboardArrowDown } from "react-icons/md";
export const ScheduleSection = ({ currentDate }) => {
    const allDates = getWeekRange();
    const [selectedDate, setSelectedDate] = useState(parseAsLocalMidnight(currentDate));
    const [isDateChooserOpened, setIsDateChooserOpened] = useState(false);
    const scrollerRef = useRef(null);
    const itemRefs = useRef([]);
    const findIndexForDate = (d) => {
        if (!d)
            return -1;
     ...
     */
    const centerItemAtIndex = (index, smooth = true) => {
        ...
        const targetScrollLeft = Math.round(container.scrollLeft + elCenterRelativeToContainerLeft - container.clientWidth / 2);
    ...
    useLayoutEffect(() => {
        ...
        const run = () => {
            if (idx >= 0)
                centerItemAtIndex(idx, false);
            else
                centerItemAtIndex(Math.floor(allDates.length / 2), false);
        };
    ...
    const TimeComponent = ({ isActive = false, day, date, }) => {
        return (<div className={`flex flex-col items-center cursor-pointer transform ${isActive ? "scale-125" : "scale-100"} transition-transform`} style={{ transformOrigin: "center center" }} // ensure scaling from center
        >
    ...
    return (<div className="flex flex-col px-2 w-full mt-3-">
            <div className="flex justify-between relative gap-2 h-fit items-center w-full">
                <p className="font-bold bg-gray-700- bg-[#007600] text-white px-2 py-[0.18rem] text-xs rounded cursor-pointer transition-all hover:bg-gray-900">
                    LIVE
                </p>

                <div ref={scrollerRef} className="w-full flex gap-8 p-4 overflow-x-auto scrollbar-none">
                    {allDates.map((date, i) => {
            const isActive = selectedDate ? date.toDateString() === selectedDate.toDateString() : false;
            return (<div key={i} 
            //@ts-ignore
            ref={(el) => (itemRefs.current[i] = el)} onClick={() => setSelectedDate(date)} className="flex-shrink-0">
                                <TimeComponent isActive={isActive} day={date.toLocaleDateString("en-US", { weekday: "short" }).toUpperCase()} date={`${date.toLocaleDateString("en-US", { month: "short" })} ${date.getDate().toString()}`}/>
                            </div>);
        })}
                </div>
            ...
            setTimeout(() => {
                ...
                {isDateChooserOpened && (<DateSelectorWindow alreadySelectedDate={selectedDate} onClose={(selected) => {
...
const DateSelectorWindow = ({ alreadySelectedDate, onClose }) => {
...
//# sourceMappingURL=ScheduleSection.jsx.map
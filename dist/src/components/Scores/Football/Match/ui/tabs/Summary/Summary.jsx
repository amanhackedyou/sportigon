...
const SummaryTab = ({ isActive }) => {
    ...
    return (<div className="w-full max-w-3xl- mx-auto bg-white rounded-2xl shadow-sm overflow-y-auto h-full">
            <div className="px-4 py-3 border-b">
                <h3 className="text-lg font-semibold">Summary</h3>
            </div>
            ...
            <div className="divide-y-">
                ...
                {groups.map((g) => (<div key={g.title} className="p-3">
                        {/* <div className="flex items-center justify-between mb-2">
                <div className="text-xs font-semibold uppercase text-gray-600">{g.title}</div>
                <div className="text-xs text-gray-500">{g.items.length} events</div>
            </div> */}
                ...
                <EventRow key={`${g.title}-${idx}-${ev.time ?? idx}`} event={ev} homeTeamId={homeTeamId} awayTeamId={awayTeamId} />
                            ...
                            {g.items.map((ev, idx) => (<EventRow key={idx} event={ev} awayTeamId={awayTeamId || 0} homeTeamId={awayTeamId || 1}/>))}
                        </div>
                    </div>))}
            </div>
        </div>);
};
const Divider = ({ status, ltext, isRounded = true }) => {
    return (<div className={`flex py-1 justify-between px-3 bg-[#e5e5e5] ${isRounded ? 'rounded' : ''} `}>
...
function getHalfLabel(timeNum) {
    ...
    if (timeNum <= 45)
        return { key: "first", title: "1st Half", order: 1 };
    if (timeNum <= 90)
        return { key: "second", title: "2nd Half", order: 2 };
    if (timeNum <= 105)
        return { key: "extra1", title: "1st Half (Extra)", order: 3 };

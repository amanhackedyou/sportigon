...
const LinesUpUI = ({ linesup, error, isLoading }) => {
    if (isLoading)
...
function Pitch({ children, svgSrc, sizeMode = "cover", className = "", }) {
    ...
    return (<section className="p-2 h-full">
            <div className={`relative aspect-[7/10]- aspect-[7/11.5] bg-transparent overflow-hidden ${className}`}>
                {/* Background SVG as <img> for URL or inline SVG component if provided */}
                {SvgComp ? (
        // Inline SVG component - scales to fill container
        <div className="absolute inset-0 pointer-events-none">
                        <SvgComp width="100%" height="100%" preserveAspectRatio={sizeMode === "cover" ? "xMidYMid slice" : "xMidYMid meet"} className="w-full h-full"/>
                    </div>) : svgSrc ? (
        // External svg/png/jpg image URL
        // Using object-fit via CSS to mimic cover/contain
        <img src={svgSrc} alt="pitch" className={`absolute inset-0 w-full h-full object-${sizeMode}`} 
        ...
        <div className="absolute inset-0" style={{
                background: "linear-gradient(180deg, rgba(4,120,87,0.08) 0%, rgba(4,120,87,0.04) 100%)",
            }}/>)}
...
/* ---------- Team render ---------- */
function TeamOnPitch({ team, side }) {
    ...
    const yFor = (rowIdx) => lerp(yStart, yEnd, totalRows === 1 ? 0.5 : rowIdx / (totalRows - 1));
    return (<>
            {/* Team badge & name */}
            {/* <div
        ...
        >
            ...
            return line.map((p, i) => {
                ...
                return (<PlayerBadge key={`${team.id}-${rowIdx}-${p.name}-${p.number}-${i}`} player={p} xPct={x} yPct={y} flipLabel={side === "bottom"}/>);
            });
        })}
        </>);
}
...
//     );
// }
function PlayerBadge({ player, xPct, yPct, flipLabel = false, }) {
    return (<div className="absolute flex flex-col items-center" style={{ left: `${xPct}%`, top: `${yPct}%`, transform: "translate(-50%, -50%)" }} title={`${player.number} ${player.name}`}>
...
//# sourceMappingURL=LinesUpUI.jsx.map
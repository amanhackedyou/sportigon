...
const TopSection = ({ tab, setTab }) => {
    const router = useRouter();
    const matchContext = useFootballMatch();
    return (<div className=" md:shadow-[rgba(0,0,0,0.12)0px_2px_8px_0px,rgba(0,0,0,0.16)0px_0px_2px_0px] flex flex-col min-h-60 justify-between relative md:mt-2rounded-lg">
            ...
            <div className="hidden mb-2 md:flex justify-between items-center mt-5- mx-10 border rounded-md">
                <button onClick={() => setTab("summary")} className={`${tab == "summary"
            ? "text-[#192036] text-lg bg-gray-200"
            : "text-[#777777] text-base"} font-medium w-full px-3 py-1 leading-none  transition-all`}>
                    Summary
                </button>

                <button onClick={() => setTab("stats")} className={`${tab == "stats"
            ? "text-[#192036] text-lg bg-gray-200"
            : "text-[#777777] text-base"} font-medium w-full px-3 py-1 leading-none  transition-all`}>
                    Stats
                </button>

                <button onClick={() => setTab("chat")} className={`${tab == "chat"
            ? "text-[#192036] text-lg bg-gray-200"
            : "text-[#777777] text-base"} font-medium w-full px-3 py-1 leading-none  transition-all`}>
                    Chat
                </button>

                <button onClick={() => setTab("line up")} className={`${tab == "line up"
            ? "text-[#192036] text-lg bg-gray-200"
            : "text-[#777777] text-base"} font-medium whitespace-nowrap w-full px-3 py-1 leading-none  transition-all`}>
                    Line up
                </button>

                <button onClick={() => setTab("standings")} className={`${tab == "standings"
            ? "text-[#192036] text-lg bg-gray-200"
            : "text-[#777777] text-base"} font-medium w-full px-3 py-1 leading-none  transition-all`}>
                    Standings
                </button>
...
const LiveMatchTopBar = () => {
    const matchContext = useFootballMatch();
    const getTeamView = (name, img) => {
        return (<div className="flex flex-col items-center h-full w-[4rem] md:w-20">
    ...
    <div className="flex justify-between items-center px-10 md:px-10">
            {getTeamView(matchContext.match?.homeTeam.name || "Unknown", matchContext.match?.homeTeam.logo || "https://static.flashscore.com/res/image/data/AZHdTBf5-GEKimEim.png")}
            ...
            {getTeamView(matchContext.match?.awayTeam.name || "Unknown", matchContext.match?.awayTeam.logo || "https://static.flashscore.com/res/image/data/AZHdTBf5-GEKimEim.png")}
        </div>
    // {/* </div> */}
    );
};
const TabView = ({ tab, setTab }) => {
    const TABS = ["Summary", "Stats", "Chat", "Line up", "Standings"];
    const TabItem = ({ tabName }) => {
        return <button onClick={() => setTab(tabName)} className={`font-medium w-fit py-1- ${tab == tabName ? "bg-[#e5e5e5]" : ""} leading-none flex flex-col-reverse gap-[0.20rem]-`}>
            <span className={`${tab == tabName ? "text-[#192036] scale-110" : "scale-100 text-[#777777]"} transition-all duration-200 leading-none my-2 px-2 whitespace-nowrap`}>
                {tabName}
            </span>
            <div className={`h-1 bg-[#007600] w-[100%]- translate-y-[-1px] rounded-full w-full transition-all ${tab === tabName ? "opacity-100" : "opacity-0"}`}></div>
        </button>;
    };
...
//# sourceMappingURL=TopSection.jsx.map